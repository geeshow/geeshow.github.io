...
#### 연관관계의 주인
데이터베이스에서 테이블의 관계를 설정할때 FK를 이용한다. JPA에서는 이 FK를 가진 테이블이 연관관계의 주인이 된다.
 여기서는 Comments가 연관관계의 주인 테이블이 된다.
...
```
 부모(Board) 객체에서 자식(Comments) 객체를 접근할 수도 있어야 하고, 자식에서 부모로 접근이 가능해야 하기 때문이다.
